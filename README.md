...
拡張機能では毎日の実行時刻と同時に開く商品タブ数（1〜100、初期値10）を設定できます。商品ページの読込は指定数まで並列化し、SQLiteへの保存は競合を避けるため1件ずつ行います。「今すぐ全商品を更新」と自動追加も同じ並列数です。ローカルアプリに接続できなかった場合は15分後に再試行します。Cloudflareのアクセス確認を検出した場合は処理を停止し、確認の回避は行いません。PCまたはEdgeが停止中の場合は実行されず、その日の指定時刻後にEdgeを起動すると未実行分を開始します。
...
- 商品ページ取得の並列数は拡張機能で設定し、データベース保存だけを直列化します。
