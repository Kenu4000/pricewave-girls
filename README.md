...
拡張機能では毎日の実行時刻を設定できます。指定時刻にEdgeと本アプリが起動していれば、登録商品を1件ずつ通常のEdgeタブで開き、15秒以上の間隔で記録します。ローカルアプリに接続できなかった場合は15分後に再試行します。Cloudflareのアクセス確認を検出した場合は処理を停止し、確認の回避は行いません。PCまたはEdgeが停止中の場合は実行されず、その日の指定時刻後にEdgeを起動すると未実行分を開始します。
