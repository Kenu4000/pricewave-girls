...
- PR #101 / ランクB・欠品等の状態違いをシリーズ商品として一切代用しない
...
- **ランクB、欠品、状態難などの状態違いProductはシリーズ価格線へ一切含めない。**
- 通常品がDBに存在しなくても、状態違い商品をその作品の代用品として使わない。
- `conditionRank === "B"`、`condition`あり、またはタイトルから状態違いと判定できる商品は除外対象。
- **通常版・廉価版・対応OS版・CD/DVD版など異なる状態Aの商品IDの価格履歴は絶対に1本へ結合しない。**
...
重要: シリーズ判定用の正規化タイトルと、グラフで表示する実商品タイトルを混同しないこと。状態違いは「edition」ではない。
...
3. 状態違いを除外し、状態Aのedition違いだけを商品ID単位の別グループとして保持する。
...
Viewer側もedition違いを商品IDごとに分離し、凡例から `#/products/[id]` へ移動できる。通常のViewer出力と、公開済みデータから組み立てるfallbackの両方で、状態違いをシリーズ線へ代用してはならない。
...
## 2026-09-04 状態違いが別商品として見えた件

`createdAt` を2026-09-04一日で確認して0件だった一方、17:59台にランクB商品の価格履歴が複数追加されていた。つまり、その時刻に新しいProductが作られたのではなく、以前からDBに存在していた状態違いProductが再取得されていた。

シリーズ実装側に「同じカタログ作品で通常品が見つからない場合は状態違いを代用する」fallbackがあったため、既存のランクB/欠品Productがシリーズの別商品として表面化した。PR #101でこのfallbackを撤廃する。

したがって、同種の表示不具合に対して `Product.createdAt` の時間帯削除を第一手にしないこと。まず「新規Product作成」なのか「既存Productの再取得」なのかを、`createdAt` と `PriceHistory.checkedAt` の両方で切り分ける。
...
状態違いしか存在しない作品について `buildSeriesProductGroups()` が空になること、Viewer fallbackに状態違いへのfallback式が復活しないことを回帰テストで固定する。
