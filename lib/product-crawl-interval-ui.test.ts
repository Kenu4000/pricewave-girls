...
test("自動巡回は長周期を均等化し、手動全件更新でも無を除外する", async () => {
  ...
  assert.match(wrapper, /crawlIntervalDays !== null/u);
  assert.match(wrapper, /products: enabledProducts/u);
...
test("popupから日次巡回ブランド設定を廃止する", async () => {
  ...
  assert.match(html, /周期構成に変更がなければ既存の均等化プラン/u);
